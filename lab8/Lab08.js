...
var number = 5;
var width = 600;
var imgNumbers = document.querySelectorAll(".buttons span");
var autoChange;//setInterval的句柄
...
function preAndNext() {
    let preBtn = document.querySelector(".arrow_left");
    let nextBtn = document.querySelector(".arrow_right");

    preBtn.onclick = function () {
        imgIndex = (imgIndex-1+number)%number;//计算前一张图片的下标
        showImg();
    };
    nextBtn.onclick = function () {
        imgIndex = (imgIndex+1)%number;//计算后一张图片的下标
        showImg();
    }
}
...
//展示图片
function showImg() {
    let num = -(imgIndex+1)*width;
    wrap.style.left = num+"px";
    for(let i=0;i<imgNumbers.length;i++){
        if(i===imgIndex){
            imgNumbers[i].setAttribute('class','on');
        }
        else{
            imgNumbers[i].setAttribute('class','');
        }
    }
...
function startPlay() {
    autoChange = setInterval(autoPlay,2000);//2s切换
}
...
function imgBtn(){
    for(let i=0;i<imgNumbers.length;i++){
        imgNumbers[i].onclick = function () {
            imgIndex =parseInt(imgNumbers[i].innerText)-1;
            showImg();
        }
    }
}
